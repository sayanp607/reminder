...
router.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query('SELECT reset_token FROM users WHERE id = $1', [userId]);
    ...
    await pool.query('UPDATE users SET password_hash = $1, reset_token = NULL WHERE id = $2', [hashedPassword, userId]);
